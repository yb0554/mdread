...
- 顶部工具栏将“添加工作区”和“收藏”改为直接可见操作；低频功能收纳到简洁的“更多”菜单，减少操作路径和视觉杂乱。
- 最近文件仅显示文件名；完整规范化路径可通过悬停查看，或右键 / `Shift+F10` 复制，不再展示 Windows `\\?\` 前缀。
- 保留 v1.2.2 的稳定性修复：文档快速切换、远程图片设置竞态、独立文档授权释放、可取消搜索和窄窗口目录抽屉。
- 正式支持的本地文档扩展名仅为 `.md` 与 `.markdown`；`.mdx`、`.mkdn`、`.mdown` 会被明确拒绝。
- Windows v1.2.3 安装包**未签名**：Windows 可能显示安全提示，下载者应通过 Release 附带的 `SHA256SUMS.txt` 校验文件完整性。
- Windows Release 同时提供安装包和便携版（`mdread_*_x64-portable.exe`）；便携版无需安装，适合已具备 WebView2 Runtime 的 Windows 系统。
